...
export class ExportService {
  ...
  async generateCsv(options: ExportOptions): Promise<string> {
    try {
      ...
      const missingBookIds = bookIds ? bookIds.filter((id) => !foundBookIds.includes(id)) : [];

      if (missingBookIds.length) {
        throw new Error(`Os seguintes IDs de livros não foram encontrados no banco de dados: ${missingBookIds.join(', ')}`);
      }
